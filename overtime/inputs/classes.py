        ...
        with open(self.path, newline='') as csvfile:
            ...
            for row in reader:
                ...
                if any(x.strip() for x in row):
                    ...
                    if 'weight' in row:
                        data['edges'][ne]['weight'] = row['weight']
                    ne += 1
...
class TflInput(Input):
    ...
    def generate(self, line_name, direction, line_stations, time):
        ...
        for name, stations in line_stations.items():
            ...
            for n in range(0, len(stations)):
                # attempt to get journeys at the current time between stations
                # along the route.
                try:
                    # make the api request.
                    journey = self.get_journey(stations[n], stations[n + 1],
                                               current_time)
                    ...
                    # update the current time to be the arrival time of the
                    # current journey.
    ...
    def get_line_routes(self, line, direction):
        """
            ...
            --------
            rdata : Dict
                A dictionary with ordered sequence of stations for each route on
                the line.
        """
        flag = False
        try:
            ...
            rdata = {}
            # for each route in the response, add the list of ordered stations
            # for that route into the dictionary.
            for route in response['orderedLineRoutes']:
                rdata[route['name']] = route['naptanIds']
        except KeyError:
            print("Key Error: response returned invalid data, client will "
    ...
    def get_journey(self, origin, destination, time, sleep_time=0):
        """
            ...
            --------
            jdata[0] : Dict
                A dictionary with information about the journey returned from
                origin to destination with departing time 'time'.
        """
        # make the api request.
        response = self.api.get_journey(origin, destination,
                                        time.replace(':', ''),
        ...
        try:
            ...
            for journey in response['journeys']:
                ...
                for leg in journey['legs']:
                    # if the selected journey is earlier than the time
                    # specified, skip it.
                    if self.convert_time(self.update_time(
                            journey['startDateTime'])) >= self.convert_time(time):
                        # filter the journey json for any required/useful
                        # information.
                        ...
                        jdata[n] = {
                            ...
                            'departurePointLocation': (
                                ...
                                leg['departurePoint']['lon']),  # departure geolocation
                            'arrivalPointLocation': (
                                leg['arrivalPoint']['lat'],
                                leg['arrivalPoint']['lon'])  # arrival geolocation
                        }
                        n += 1
        except KeyError:
            print("Key Error: response returned invalid data, client will "
        ...
        try:
            with open(self.jpath, 'w') as csvfile:
                writer = csv.DictWriter(csvfile,
                                        fieldnames=cols)  # initialize csv writer.
                writer.writeheader()
